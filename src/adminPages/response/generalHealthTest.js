...
const GeneralHealthTestPanel = () => {
  const [generalHealthTests, setGeneralHealthTests] = useState([]);
  ...
  useEffect(() => {
    const fetchDepressionTests = async () => {
      try {
        ...
        const processedDepressionTests = generalHealthTestData.map(test => ({
          id: test.id || null,
        ...
        setGeneralHealthTests(sortedTests);
      } catch (error) {
        setError(error.message || 'Failed to fetch generalHealth tests');
        setGeneralHealthTests([]);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return <div className="loading-container">Loading generalHealth test submissions...</div>;
  }
  ...
  const handleView = (id) => {
    navigate(`/admin/responses/generalHealthTest/${id}`);
  };

  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this generalHealth test submission?')) {
      try {
        await deleteDataBS('/generalHealthtest', id);
        
        setGeneralHealthTests(prevTests => 
          prevTests.filter(test => test.id !== id)
        );
      } catch (error) {
        console.error('Error deleting generalHealth test:', error);
        alert('Failed to delete generalHealth test.');
      }
    }
  };
  ...
  return (
    <div className="admin-container">
      ...
      {generalHealthTests.length === 0 ? (
        <div className="no-data-container">
          <p className="no-data">No generalHealth test submissions found.</p>
...
export default GeneralHealthTestPanel;